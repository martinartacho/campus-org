                    ...
                    <form action="{{ route('admin.events.update', $event) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2 border-t pt-4 mt-4">
                                <h4 class="text-lg font-medium text-gray-900 mb-2">{{ __('site.Recurrence Settings') }}</h4>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <select name="recurrence_type" id="recurrence_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                            <option value="none" {{ old('recurrence_type', $event->recurrence_type) == 'none' ? 'selected' : '' }}>{{ __('site.None') }}</option>
                                            <option value="daily" {{ old('recurrence_type', $event->recurrence_type) == 'daily' ? 'selected' : '' }}>{{ __('site.Daily') }}</option>
                                            <option value="weekly" {{ old('recurrence_type', $event->recurrence_type) == 'weekly' ? 'selected' : '' }}>{{ __('site.Weekly') }}</option>
                                            <option value="monthly" {{ old('recurrence_type', $event->recurrence_type) == 'monthly' ? 'selected' : '' }}>{{ __('site.Monthly') }}</option>
                                            <option value="yearly" {{ old('recurrence_type', $event->recurrence_type) == 'yearly' ? 'selected' : '' }}>{{ __('site.Yearly') }}</option>
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 flex rounded-md shadow-sm">
                                            <input type="number" name="recurrence_interval" id="recurrence_interval" min="1" value="{{ old('recurrence_interval', $event->recurrence_interval) }}" class="focus:ring-indigo-500 focus:border-indigo-500 block w-20 rounded-none rounded-l-md sm:text-sm border-gray-300">
                                    ...
                                    <div>
                                        ...
                                        <input type="date" name="recurrence_end_date" id="recurrence_end_date" value="{{ old('recurrence_end_date', $event->recurrence_end_date ? $event->recurrence_end_date->format('Y-m-d') : '') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    </div>
                                    
                                    <div>
                                        <label for="recurrence_count" class="block text-sm font-medium text-gray-700">{{ __('site.Number of Occurrences') }}</label>
                                        <input type="number" name="recurrence_count" id="recurrence_count" min="1" value="{{ old('recurrence_count', $event->recurrence_count) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        @if ($event->recurrence_type !== 'none' || $event->parent_id === $event->id)
