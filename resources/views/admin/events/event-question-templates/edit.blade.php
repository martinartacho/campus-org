<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('site.Edit Question Template') }}: {{ $template->template_name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    {{-- <form action="{{ route('admin.event-question-templates.update') }}" method="POST"> --}}
                    <form action="{{ route('admin.event-question-templates.update', $template->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="template_description" id="template_description" rows="3"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('template_description', $template->template_description) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="question" id="question" value="{{ old('question', $template->question) }}" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            @error('question')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <select name="type" id="type" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                <option value="text" {{ old('type', $template->type) == 'text' ? 'selected' : '' }}>{{ __('site.Text') }}</option>
                                <option value="single" {{ old('type', $template->type) == 'single' ? 'selected' : '' }}>{{ __('site.Single Choice') }}</option>
                                <option value="multiple" {{ old('type', $template->type) == 'multiple' ? 'selected' : '' }}>{{ __('site.Multiple Choice') }}</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4" id="options-container" style="{{ in_array(old('type', $template->type), ['single', 'multiple']) ? 'block' : 'none' }};">
                        ...
                        <div class="mb-4">
                            <div class="flex items-center">
                                <input type="checkbox" name="required" id="required" value="1" {{ old('required', $template->required) ? 'checked' : '' }}
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @elseif(isset($template) && $template->options)
            const currentOptions = {!! json_encode($template->options) !!};
