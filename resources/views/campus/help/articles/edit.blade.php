...
@section('title', 'Editar Article d\'Ajuda')
@section('subtitle', 'Editar article d\'ajuda existent')
...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Editar Article d\'Ajuda</h1>
        <div class="flex space-x-2">
            <a href="{{ route('campus.help.articles.show', $helpArticle) }}" 
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <div class="p-6">
            <form method="POST" action="{{ route('campus.help.articles.update', $helpArticle) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                               value="{{ old('title', $helpArticle->title) }}" required>
                        @error('title')
                    ...
                    <div>
                        ...
                        <select name="area" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                            @foreach($areas as $key => $label)
                                <option value="{{ $key }}" {{ old('area', $helpArticle->area) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="context" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                               value="{{ old('context', $helpArticle->context) }}" placeholder="Context on s'aplica aquest article">
                        @error('context')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="type" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                               value="{{ old('type', $helpArticle->type) }}" placeholder="Tipus d'article">
                        @error('type')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select name="help_category_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" 
                                        {{ old('help_category_id', $helpArticle->help_category_id) == $category->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                            @foreach($statuses as $key => $label)
                                <option value="{{ $key }}" {{ old('status', $helpArticle->status) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="order" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                               value="{{ old('order', $helpArticle->order) }}" min="0">
                        @error('order')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="content" id="content-editor" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" rows="10" required>{{ old('content', $helpArticle->content) }}</textarea>
                    @error('content')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="font-medium">Creat per:</span> {{ $helpArticle->createdBy?->name ?? '-' }}
                        </div>
                        <div>
                            <span class="font-medium">Creat el:</span> {{ $helpArticle->created_at->format('d/m/Y H:i') }}
                        </div>
                        <div>
                            <span class="font-medium">Actualitzat per:</span> {{ $helpArticle->updatedBy?->name ?? '-' }}
                        </div>
                        <div>
                            <span class="font-medium">Actualitzat el:</span> {{ $helpArticle->updated_at->format('d/m/Y H:i') }}
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-4 mt-6">
                    ...
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200">
                        <i class="bi bi-check-circle mr-2"></i>Actualitzar Article
                    </button>
                </div>
            </form>
        </div>
