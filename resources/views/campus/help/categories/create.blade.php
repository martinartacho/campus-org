...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Crear Categoria d\'Ajuda</h1>
            ...
            <form method="POST" action="{{ route('campus.help.categories.store') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="area" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            <option value="cursos" {{ old('area') == 'cursos' ? 'selected' : '' }}>Cursos</option>
                            <option value="matricula" {{ old('area') == 'matricula' ? 'selected' : '' }}>Matrícula</option>
                            <option value="materiales" {{ old('area') == 'materiales' ? 'selected' : '' }}>Materiales</option>
                            <option value="configuracion" {{ old('area') == 'configuracion' ? 'selected' : '' }}>Configuración</option>
                        </select>
                        @error('area')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                               value="{{ old('icon', 'bi-question-circle') }}" placeholder="bi-question-circle">
                        @error('icon')
                            <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                        @enderror
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
                              rows="4" placeholder="Descripció de la categoria...">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-4 mt-6">
                    ...
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200">
                        <i class="bi bi-check-circle mr-2"></i>Crear Categoria
                    </button>
                </div>
            </form>
        </div>
