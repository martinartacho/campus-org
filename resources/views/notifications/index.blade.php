                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    @endcan
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-32">
                                        {{ __('site.Actions') }}
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($notifications as $notification)
                                    <tr>
                                        <td class="px-4 py-4">
                                            <div class="text-sm font-medium text-gray-900 truncate max-w-xs" title="{{ $notification->title }}">
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        ...
                                        @can('publish-notification')
                                            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 truncate max-w-xs" title="{{ $notification->sender->name }}">
                                                {{ Str::limit($notification->sender->name, 15) }}
                                            </td>
                                            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                                                @if($notification->recipient_type === 'all')
                                                    <span class="text-xs">{{ __('site.All_users') }}</span>
                                                @elseif($notification->recipient_type === 'role')
                                                    <span class="text-xs">{{ Str::limit($notification->recipient_role, 10) }}</span>
                                                @else 
                                                    <span class="text-xs">
                                                ...
                                                @if($notification->is_published)
                                                    @if($notification->email_pending_count > 0)
                                                        <form method="POST" action="{{ route('notifications.send-email', $notification) }}" 
                                                            ...
                                                            @csrf
                                                            <button type="submit" class="text-green-600 hover:text-green-900 p-1 rounded hover:bg-green-50" title="{{ __('site.Send_email') }}">
                                                    ...
                                                    @if($notification->web_pending_count > 0)
                                                        <form method="POST" action="{{ route('notifications.send-web', $notification) }}" 
                                                            class="inline send-form">
                                                            @csrf
                                                            <button type="submit" class="text-blue-600 hover:text-blue-900 p-1 rounded hover:bg-blue-50" title="{{ __('site.Send_web') }}">
                                                    ...
                                                    @if($notification->push_pending_count > 0)
                                                        <form method="POST" action="{{ route('notifications.send-push', $notification) }}" 
                                                            class="inline send-form">
                                                            @csrf
                                                            <button type="submit" class="text-purple-600 hover:text-purple-900 p-1 rounded hover:bg-purple-50" title="{{ __('site.Send_push') }}">
