...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <ol class="flex items-center space-x-2 text-sm text-gray-500">
            <li>
                <a href="{{ route('catalog.index') }}" class="hover:text-blue-600">
                    <i class="bi bi-house"></i> {{ __('campus.catalog') }}
                </a>
            </li>
                    ...
                    <div class="flex flex-wrap gap-4 text-white">
                        @if($course->hours)
                            <div class="flex items-center">
                                ...
                                <span>{{ $course->hours }} {{ __('campus.hours') }}</span>
                            </div>
                        @endif
                        @if($course->sessions)
                            <div class="flex items-center">
                                <i class="bi bi-calendar-week me-2"></i>
                                <span>{{ $course->sessions }} {{ __('campus.sessions') }}</span>
                            </div>
                        @endif
                        @if($course->location)
                            <div class="flex items-center">
                ...
                <div class="mt-6 md:mt-0 text-center md:text-right">
                    <div class="text-white mb-4">
                        @if($course->price > 0)
                            ...
                            <div class="text-blue-100">{{ __('campus.total_price') }}</div>
                        @else
                            <div class="text-4xl font-bold">{{ __('campus.free_course') }}</div>
                            <div class="text-blue-100">{{ __('campus.no_cost') }}</div>
                        @endif
                    </div>
                    ...
                    @else
                        <button onclick="addToCart({{ $course->id }})" 
                            ...
                            <i class="bi bi-cart-plus me-2"></i>{{ __('campus.add_to_cart') }}
                        </button>
                    @endif
                    
                    <div class="mt-3">
                        <a href="{{ route('cart.index') }}" class="text-white hover:text-blue-200 underline text-sm">
                            <i class="bi bi-cart me-1"></i>{{ __('campus.view_cart') }}
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            @if($course->description)
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">
                        <i class="bi bi-file-text me-2"></i>{{ __('campus.course_description') }}
                    </h2>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <h2 class="text-2xl font-bold text-gray-900 mb-4">
                    <i class="bi bi-calendar3 me-2"></i>{{ __('campus.calendar_schedule') }}
                </h2>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h3 class="font-semibold text-gray-800 mb-3">{{ __('campus.course_dates') }}</h3>
                        <div class="space-y-2">
                            <div class="flex items-center text-gray-600">
                                <i class="bi bi-calendar-event me-2 text-blue-500"></i>
                                <span><strong>{{ __('campus.start') }}:</strong> {{ $course->start_date?->format('d/m/Y') ?? 'Por determinar' }}</span>
                            </div>
                            <div class="flex items-center text-gray-600">
                                <i class="bi bi-calendar-event me-2 text-blue-500"></i>
                                <span><strong>{{ __('campus.end') }}:</strong> {{ $course->end_date?->format('d/m/Y') ?? 'Por determinar' }}</span>
                            </div>
                            @if($course->season)
                                <div class="flex items-center text-gray-600">
                                    <i class="bi bi-calendar-range me-2 text-blue-500"></i>
                                    <span><strong>{{ __('campus.season') }}:</strong> {{ $course->season->name }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-800 mb-3">{{ __('campus.additional_info') }}</h3>
                        <div class="space-y-2">
                            @if($course->hours)
                                <div class="flex items-center text-gray-600">
                                    <i class="bi bi-clock me-2 text-blue-500"></i>
                                    <span><strong>{{ __('campus.duration') }}:</strong> {{ $course->hours }} {{ __('campus.hours') }}</span>
                                </div>
                            @endif
                            @if($course->sessions)
                                <div class="flex items-center text-gray-600">
                                    <i class="bi bi-calendar-week me-2 text-blue-500"></i>
                                    <span><strong>{{ __('campus.sessions') }}:</strong> {{ $course->sessions }}</span>
                                </div>
                            @endif
                            @if($course->location)
                                <div class="flex items-center text-gray-600">
                                    <i class="bi bi-geo-alt me-2 text-blue-500"></i>
                                    <span><strong>{{ __('campus.location') }}:</strong> {{ $course->location }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                @if($course->schedule && !empty($course->schedule))
                    <div class="mt-6">
                        <h3 class="font-semibold text-gray-800 mb-3">{{ __('campus.detailed_schedule') }}</h3>
            ...
            @if($course->requirements)
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">
                        <i class="bi bi-list-check me-2"></i>{{ __('campus.previous_requirements') }}
                    </h2>
            ...
            @if($course->objectives)
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">
                        <i class="bi bi-trophy me-2"></i>{{ __('campus.course_objectives') }}
                    </h2>
                    <div class="text-gray-700">
            ...
            @if($course->teachers && $course->teachers->count() > 0)
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">
                        <i class="bi bi-people me-2"></i>{{ __('campus.teaching_staff') }}
                    </h2>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">
                    <i class="bi bi-info-circle me-2"></i>{{ __('campus.enrollment_info') }}
                </h3>
                ...
                <div class="space-y-4">
                    ...
                    @if($course->season)
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-gray-600">{{ __('campus.enrollment_period') }}:</span>
                                @if($course->season->isRegistrationOpen())
                                    <span class="text-green-600 font-semibold">{{ __('campus.open') }}</span>
                                @else
                                    <span class="text-red-600 font-semibold">{{ __('campus.closed') }}</span>
                                @endif
                            </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600">{{ __('campus.course_status') }}:</span>
            ...
            @if($course->category)
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">
                        <i class="bi bi-tag me-2"></i>{{ __('campus.category') }}
                    </h3>
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">{{ __('campus.main_category') }}</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if($relatedCourses->count() > 0)
                <div class="bg-white rounded-lg shadow-md p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">
                        <i class="bi bi-collection me-2"></i>{{ __('campus.related_courses') }}
                    </h3>
                    <div class="space-y-3">
                                ...
                                <a href="{{ route('catalog.show', $relatedCourse) }}" 
                                   class="block hover:text-blue-600 transition">
                                    ...
                                    <div class="flex items-center space-x-3 text-sm text-gray-600 mt-1">
                                        ...
                                        @else
                                            <span>{{ __('campus.free_course') }}</span>
                                        @endif
                                    </div>
                                </a>
                            </div>
