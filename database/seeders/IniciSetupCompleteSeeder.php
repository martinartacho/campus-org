...
class IniciSetupCompleteSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        foreach ($seeders as $seederClass => $info) {
            $this->command->info('');
            $this->command->info("📋 {$info['name']}");
            $this->command->info("   {$info['description']}");
            ...
            if ($execute) {
                try {
                    ...
                    $this->command->error("   ❌ Error en {$seederClass}: " . $e->getMessage());
                    $errors[] = [
                        'seeder' => $seederClass,
                        'name' => $info['name'],
                        'error' => $e->getMessage()
    ...
    private function printFinalReport($executed, $skipped, $errors)
    {
        ...
        if (!empty($executed)) {
            ...
            foreach ($executed as $item) {
                $this->command->info("   - {$item['name']} ({$item['seeder']}) - {$item['duration']}s");
            }
        }
        ...
        if (!empty($skipped)) {
            ...
            foreach ($skipped as $item) {
                $this->command->info("   - {$item['name']} ({$item['seeder']})");
            }
        }
